...
- Name: smoke.spec.ts >> Golden-path smoke test >> sidebar shows all navigation items after login
- Location: e2e\smoke.spec.ts:99:3
...
- alert [ref=e3]:
  - generic [ref=e4]:
    ...
    - generic [ref=e13]:
      ...
      - paragraph [ref=e15]: HMS-MRE8LJYZ-F4
    - group [ref=e16]:
...
```ts
  1   | /**
  2   |  * Golden-path smoke test: app loads → sidebar visible → key pages render.
  3   |  *
  4   |  * Tauri `invoke` is stubbed to return mock data so the frontend renders
  5   |  * without a real backend. True desktop E2E (real Postgres + Tauri) requires
  6   |  * `tauri-driver` — see e2e/README.md.
  7   |  */
  8   | import { test, expect } from "@playwright/test";
  9   | 
  10  | /** Stub `invoke` to return mock data for key commands. */
  11  | async function stubTauriInvoke(page: import("@playwright/test").Page) {
  12  |   await page.addInitScript(() => {
  13  |     const mockData: Record<string, unknown> = {
  14  |       get_config: {
  15  |         mode: "server",
  16  |         db_host: "127.0.0.1",
  17  |         db_port: 5432,
  18  |         db_user: "postgres",
  19  |         db_name: "hospital_db",

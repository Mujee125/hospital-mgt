...
async fn test_sec005_unique_unit_number() {
    ...
    let result = sqlx::query(
        r#"INSERT INTO blood_units
              (unit_number, donor_id, component_type, blood_group, rh_factor,
               volume_ml, expiry_date, status)
           VALUES ('BU-DUP-TEST', $1, 'whole_blood', 'O', '-', 450, NOW() + INTERVAL '35 days', 'quarantine')"#,
    )
    .bind(donor_id)
    .execute(&pool)
    .await;
    assert!(result.is_ok(), "First insert with unique number must succeed");
...
async fn test_sec007_volume_boundary_minimum() {
    ...
    let result = sqlx::query(
        r#"INSERT INTO blood_units
              (unit_number, donor_id, component_type, blood_group, rh_factor,
               volume_ml, expiry_date, status)
           VALUES ('BU-VOL-1', $1, 'whole_blood', 'O', '-', 1, NOW() + INTERVAL '35 days', 'quarantine')"#,
    )
    .bind(donor_id)
    .execute(&pool)
    .await;
    assert!(result.is_ok(), "volume_ml=1 must be accepted (boundary)");
}
...
async fn test_sec008_patients_rh_factor_check() {
    ...
    let result = sqlx::query(
        r#"INSERT INTO patients (first_name, last_name, blood_group, rh_factor)
           VALUES ('Test', 'Patient', 'O', 'positive')"#,
    )
    .execute(&pool)
    .await;
    assert!(result.is_err(), "chk_patients_rh_factor must reject 'positive'");
}
...
async fn test_sec010_fk_nonexistent_donor() {
    ...
    let result = sqlx::query(
        r#"INSERT INTO blood_units
              (unit_number, donor_id, component_type, blood_group, rh_factor,
               volume_ml, expiry_date, status)
           VALUES ('BU-FK-1', 999999, 'whole_blood', 'O', '-', 450, NOW() + INTERVAL '35 days', 'quarantine')"#,
    )
    .execute(&pool)
    .await;
...
async fn test_sec011_max_volume_boundary() {
    ...
    let result = sqlx::query(
        r#"INSERT INTO blood_donations
           ...
           VALUES ('BDN-MAX-600', $1, 600, 'O', '-', 'collected', 'pending')"#,
    )
    .bind(donor_id)
    .execute(&pool)
    .await;
    assert!(result.is_ok(), "volume_ml=600 must be accepted (boundary)");
}

// IT-001 / AERP Part G: when compiling the test target, the crate's private
// modules must be visible to the integration tests in tests/ (which are
// separate crates). `#[cfg(test)]`-gated `pub` changes NOTHING in release or
// dev binaries — it only applies while building the library for its own unit
// tests... but integration tests build the lib WITHOUT --test, so gate on the
// cargo `hms-integration-tests` feature instead, enabled only via
// `cargo test --features hms-integration-tests --tests`.

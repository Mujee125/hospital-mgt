...
/// IPC-09 input checks for `send_whatsapp_to_patient` (AERP Part G extraction):
/// (1) recipient phone MUST belong to a registered, non-deleted patient, and
/// (2) message body capped at MAX_MESSAGE_LEN. Pure DB + string logic — no
/// AppHandle — so integration tests can fuzz these guards directly
/// (WP1-P02). The RBAC guard runs in the command BEFORE this function.
pub async fn send_to_patient_checks(
    pool: &sqlx::PgPool,
    phone: &str,
    message: &str,
) -> Result<(), String> {
    ...
    // soft-deleted patient's stale phone record must NOT be usable as a
    // send target.

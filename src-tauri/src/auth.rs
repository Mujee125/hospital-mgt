...
/// Argon2id with m=19456 KiB is memory- and CPU-hard (~50-150 ms per op).
/// These are BLOCKING operations — running them on a tokio worker thread
/// stalls every other task scheduled on that worker (including the sqlx
/// connection pool's I/O), which under concurrent logins caused
/// PoolTimedOut cascades (found by AERP Part G WP2-C01 tests). Callers in
/// async contexts use the _async variants below.
...
/// Async wrappers: run the memory-hard work on the blocking thread pool
/// (`spawn_blocking`), keeping the async workers free for I/O.
pub async fn hash_password_async(plain: &str) -> Result<String, String> {
...
pub async fn verify_password_async(plain: &str, phc: &str) -> bool {
    let (p, h) = (plain.to_string(), phc.to_string());
...
pub async fn seed_defaults(pool: &PgPool) -> Result<(), String> {
    ...
    if user_count.0 == 0 {
        ...
        let hash = hash_password_async(&bootstrap_password).await?;
...
/// WP-2 login logic core — everything the `login` command does except the
/// AppHandle-dependent WARN log and the `session_invalidated` event emission.
/// Extracted (AERP Part G) so integration tests can exercise the full
/// authentication + session lifecycle against a live PostgreSQL without a
/// Wry AppHandle: Argon2 verification, lockout, single-active-session
/// invalidation, session-row creation, `load_session`, and audit records.
pub async fn login_core(
    ...
    let (user_id, username, full_name, hash, is_active, must_change, failed, locked_until) =
        match row {
            ...
            None => {
                ...
                let _ = verify_password_async(&request.password, "$argon2id$v=19$m=19456,t=2,p=1$AAAAAAAAAAAAAAAAAAAAAAAAAAAA$AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA").await;
                ...
                // patterns, but a free-form format wouldn't be caught by the
                // redactor. The command wrapper logs a generic message.
    ...
    if !verify_password_async(&request.password, &hash).await {
...
pub async fn login(
    ...
    // AERP Part G core extraction: all authentication/session/audit logic
    // lives in `login_core`; this wrapper adds only the AppHandle concerns.
    let result = login_core(pool.inner(), &session_state, request).await;
    match &result {
        Ok(resp) => {
            // WP-2.2 Layer 2: emit session_invalidated so other PCs running
...
pub async fn change_password(
    ...
    if !verify_password_async(&request.current_password, &hash.0).await {
...
pub async fn create_user(
    ...
    let hash = hash_password_async(&request.password).await?;
...
/// WP-2.2 admin user-update core (AERP Part G extraction): guards + DB writes
/// + audit, without the AppHandle event emission.
pub async fn update_user_core(
    pool: &PgPool,
    session_state: &SessionState,
    request: UpdateUserRequest,
) -> Result<(), String> {
    let session = rbac::require_strong(session_state, pool, Permission::UsersManage).await?;
...
pub async fn update_user(
    ...
    let emits = request.is_active == Some(false) || request.roles.is_some();
...
/// WP-2.2 password-reset core (AERP Part G extraction): guard + hash update +
/// session invalidation + audit, without the AppHandle event emission.
pub async fn reset_user_password_core(
    pool: &PgPool,
    session_state: &SessionState,
    id: i32,
    new_password: String,
    ...
    let hash = hash_password_async(&new_password).await?;

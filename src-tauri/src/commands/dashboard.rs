...
async fn count(pool: &PgPool, sql: &str) -> i64 {
    let r: Result<(i64,), sqlx::Error> = sqlx::query_as::<_, (i64,)>(sql).fetch_one(pool).await;
...
pub async fn get_dashboard_kpis(
    ...
    let (appointments_today, appointments_scheduled, appointments_completed) = if s
        .has(Permission::AppointmentsView)
    {
        let today = count(
            pool,
    ...
    let pending_lab_orders = if s.has(Permission::LabView) {
        count(
            pool,
            "SELECT COUNT(*) FROM lab_orders WHERE status IN ('ordered','collected','in-progress')",
        )
        .await
    } else {
        0
